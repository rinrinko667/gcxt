...
def evaluate(opt, device):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            output = model.forward_for_eval(batch, pred_scale=None)

            pred_pose = output["head_pose"][0].detach().cpu()
            gt_pose = output["gt_head_pose"][0].detach().cpu()
...
def parse_opt():
    ...
    parser.add_argument("--output_json", type=str, default="outputs/gravitynet_custom_eval.json")
