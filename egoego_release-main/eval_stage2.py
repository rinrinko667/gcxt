...
def test(opt, device):
    ...
    if opt.full_body_gt_data_path:
        full_body_gt_data_path = opt.full_body_gt_data_path
    else:
        full_body_gt_data_path = os.path.join(
            opt.data_root_folder, "amass_same_shape_egoego_processed", "test_amass_smplh_motion.p"
        )

    if not os.path.exists(full_body_gt_data_path):
        raise FileNotFoundError(f"Cannot find full-body GT file: {full_body_gt_data_path}")
    ...
    if opt.diffusion_weight_path:
        diffusion_weight_path = opt.diffusion_weight_path
    else:
        weight_root_folder = "./pretrained_models"
        diffusion_weight_path = os.path.join(weight_root_folder, "stage2_diffusion_4.pt")

    if not os.path.exists(diffusion_weight_path):
        raise FileNotFoundError(f"Cannot find diffusion weight: {diffusion_weight_path}")
    ...
    eval_subset_keywords = [k.strip() for k in opt.eval_subset_keywords.split(",") if k.strip()]
    ...
    with torch.no_grad():
        for k in full_body_gt_data:
            ...
            if (not opt.eval_all_sequences) and eval_subset_keywords:
                if not any(key in seq_name for key in eval_subset_keywords):
                    continue
    ...
    if len(e_root_list) == 0:
        raise RuntimeError(
            "No sequence was evaluated. "
            "Use --eval_all_sequences, or set --eval_subset_keywords to match your seq_name."
        )
...
def parse_opt():
    ...
    parser.add_argument(
        "--full_body_gt_data_path",
        type=str,
        default="",
        help="Optional explicit path to test_amass_smplh_motion.p. If empty, use data_root_folder/amass_same_shape_egoego_processed/test_amass_smplh_motion.p",
    )
    parser.add_argument(
        "--diffusion_weight_path",
        type=str,
        default="",
        help="Optional explicit diffusion checkpoint path. If empty, use ./pretrained_models/stage2_diffusion_4.pt",
    )
    parser.add_argument(
        "--eval_all_sequences",
        action="store_true",
        help="Evaluate all sequences without filtering by name keyword.",
    )
    parser.add_argument(
        "--eval_subset_keywords",
        type=str,
        default="Transitions_mocap,HumanEva",
        help="Comma-separated seq_name keywords used when eval_all_sequences is not set.",
    )
