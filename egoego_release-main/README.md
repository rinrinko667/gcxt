...
```
Install system dependencies for `mujoco-py` build/runtime (Ubuntu).
```
sudo apt-get update
sudo apt-get install -y libosmesa6-dev patchelf libgl1-mesa-glx libglfw3
```
Install PyTorch. 
```
...
```
If `mujoco-py` reports Cython compile issues, make sure Cython is pinned:
```
pip install --force-reinstall "Cython<3"
```
...
If you would like to generate visualizations, install [Blender](https://www.blender.org/download/) and set `BLENDER_PATH` to the blender binary path.
...
Then run EgoEgo pipeline on the testing data.
```
sh scripts/test_egoego_pipeline.sh
```
By default, the script runs without visualization. To enable visualization:
```
export BLENDER_PATH=/path/to/blender
EGOEGO_GEN_VIS=1 sh scripts/test_egoego_pipeline.sh
```
...
```
sh scripts/stage12/eval_stage2.sh 
```
...
### Training
Custom-data related code is centralized in `custom_pipeline/`, and corresponding shell entrypoints are in `scripts/stage12/`.

For custom-data reproduction that is consistent with the official stage1 input style (optical flow + SLAM trajectory):

1. Put your RGB-derived flow under `flow_out/` (for example `000000_000001.flo`, ...).
2. Put your DROID trajectory files under `output_dir/` (supported default patterns: `output_dir/{seq_name}/reconstruction.pth`, `output_dir/{seq_name}.pth`, `output_dir/{seq_name}.npy`, or fallback `output_dir/reconstruction.pth`).
3. Run:
```
sh scripts/stage12/prepare_mydataset_stage1.sh
sh scripts/stage12/prepare_mydataset_stage2.sh
```
4. Train/eval/infer with scripts in `scripts/stage12/`.

Note: current custom scripts default to raw flow-map input (no `--input_of_feats`).  
If you convert stage1 data with `--use_of_feats`, add `--input_of_feats` for HeadNet train/eval/infer.
...
```
sh scripts/stage12/train_full_body_cond_diffusion.sh 
```

#### Training GravityNet 
```
sh scripts/stage12/train_gravitynet.sh
```

#### Training HeadNet 
Train HeadNet on ARES.
``` 
sh scripts/stage12/train_headnet_on_ares.sh
```

Train HeadNet on GIMO. 
```
sh scripts/stage12/train_headnet_on_gimo.sh
```

Train HeadNet on Kinpoly-Realworld. 
```
sh scripts/stage12/train_headnet_on_kinpoly.sh
```
